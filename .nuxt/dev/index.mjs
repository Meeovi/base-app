...
import { defineEventHandler as defineEventHandler$1, readBody as readBody$1 } from 'file://C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/layers/auth-app/node_modules/h3/dist/index.mjs';
import admin from 'file://C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/layers/auth-app/node_modules/firebase-admin/lib/index.js';
...
import consola, { consola as consola$1 } from 'file://C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/node_modules/consola/dist/index.mjs';
...
import { parse } from 'file://C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/node_modules/cookie-es/dist/index.mjs';
...
const serverAssets = [{"baseName":"server","dir":"C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/server/assets"}];
  ...
  "ipx": {
    ...
    "fs": {
      "dir": [
        ...
        "C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/layers/search-app/public",
        "C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/layers/media-app/public",
        ...
        "C:/Users/Basti/Documents/My_Websites/Handmade_Sites/Javascript-Projects/Nuxt-Projects/Meeovi_Framework/Ecosystem/base-app/layers/auth-app/public"
      ]
    },
    "http": {
      "domains": []
...
function getFirebaseAdmin() {
  ...
  const svcPath = process.env.FIREBASE_SERVICE_ACCOUNT_PATH || join(process.cwd(), "service-account.json");
  if (existsSync(svcPath)) {
    const raw = readFileSync(svcPath, "utf-8");
  ...
  try {
    ...
    throw new Error("Unable to initialize Firebase Admin SDK. Provide service account file or env var.");
  }
}
...
const _3MA6LX = lazyEventHandler(() => {
...
const _lazy_3e9CrW = () => Promise.resolve().then(function () { return customers_post$1; });
const _lazy_D2pOnC = () => Promise.resolve().then(function () { return user_post$1; });
...
const handlers = [
  ...
  { route: '/api/magento/customers', handler: _lazy_3e9CrW, lazy: true, middleware: false, method: "post" },
  { route: '/api/sync/user', handler: _lazy_D2pOnC, lazy: true, middleware: false, method: "post" },
...
const uploadVideo_post = defineEventHandler(async (event) => {
  ...
  const s3 = new S3Client({
    ...
    region: "us-east-1",
    credentials: {
      accessKeyId: process.env.MINIO_USER || "MINIO_USER",
      secretAccessKey: process.env.MINIO_PASS || "MINIO_PASS"
    },
    forcePathStyle: true
  });
  await s3.send(new PutObjectCommand({
    Bucket: "vibez-uploads",
    Key: minioKey,
    Body: fileBuffer,
...
const users_post = defineEventHandler$1(async (event) => {
  ...
  const directusUrl = process.env.DIRECTUS_API_URL || process.env.NUXT_PRIVATE_DIRECTUS_API_URL;
  const directusToken = process.env.DIRECTUS_ADMIN_TOKEN || process.env.NUXT_PRIVATE_DIRECTUS_ADMIN_TOKEN;
  if (!directusUrl || !directusToken) {
    event.res.statusCode = 500;
    return { error: "Directus configuration missing" };
  }
  try {
    ...
    const queryUrl = `${directusUrl.replace(/\/$/, "")}/items/users?filter[email][_eq]=${encodeURIComponent(email)}&limit=1`;
    const findRes = await fetch(queryUrl, {
      method: "GET",
      headers: { Authorization: `Bearer ${directusToken}` }
    });
    const found = await findRes.json();
    if (findRes.ok && (found == null ? void 0 : found.data) && found.data.length) {
      const id = found.data[0].id;
      const updateUrl = `${directusUrl.replace(/\/$/, "")}/items/users/${id}`;
      ...
      if (!updateRes.ok) {
        event.res.statusCode = updateRes.status || 500;
        return { error: updateData };
      }
      return updateData;
    }
    const createUrl = `${directusUrl.replace(/\/$/, "")}/items/users`;
    ...
    if (!createRes.ok) {
      event.res.statusCode = createRes.status || 500;
      return { error: createData };
    }
    return createData;
  } catch (err) {
    event.res.statusCode = 500;
    return { error: (err == null ? void 0 : err.message) || "Directus request failed" };
  }
});
...
const customers_post = defineEventHandler$1(async (event) => {
  const admin = getFirebaseAdmin();
  const authHeader = event.node.req.headers.authorization || "";
  const idToken = authHeader.startsWith("Bearer ") ? authHeader.split(" ")[1] : null;
  if (!idToken) {
  ...
  const body = await readBody$1(event);
  const magentoUrl = process.env.MAGENTO_API_URL || process.env.NUXT_PRIVATE_MAGENTO_API_URL || process.env.NUXT_PUBLIC_MAGENTO_API_URL;
  const magentoToken = process.env.MAGENTO_API_TOKEN || process.env.NUXT_PRIVATE_MAGENTO_API_TOKEN;
  if (!magentoUrl || !magentoToken) {
    event.res.statusCode = 500;
    return { error: "Magento configuration missing" };
  }
  try {
    const resp = await fetch(`${magentoUrl.replace(/\/$/, "")}/rest/V1/customers`, {
      method: "POST",
      headers: {
    ...
    if (!resp.ok) {
      event.res.statusCode = resp.status || 500;
      return { error: data };
    }
    return data;
  } catch (err) {
    event.res.statusCode = 500;
    return { error: (err == null ? void 0 : err.message) || "Magento request failed" };
  }
});
...
const user_post = defineEventHandler$1(async (event) => {
  var _a, _b;
  try {
    const admin = getFirebaseAdmin();
    const authHeader = event.node.req.headers.authorization || "";
    ...
    const directusUrl = process.env.DIRECTUS_API_URL || process.env.NUXT_PRIVATE_DIRECTUS_API_URL;
    const directusToken = process.env.DIRECTUS_ADMIN_TOKEN || process.env.NUXT_PRIVATE_DIRECTUS_ADMIN_TOKEN;
    const magentoUrl = process.env.MAGENTO_API_URL || process.env.NUXT_PRIVATE_MAGENTO_API_URL;
    const magentoToken = process.env.MAGENTO_API_TOKEN || process.env.NUXT_PRIVATE_MAGENTO_API_TOKEN;
    if (!directusUrl || !directusToken) {
      event.res.statusCode = 500;
      return { error: "Directus configuration missing" };
    }
    const findUrl = `${directusUrl.replace(/\/$/, "")}/items/users?filter[email][_eq]=${encodeURIComponent(email)}&limit=1`;
    const findRes = await fetch(findUrl, { method: "GET", headers: { Authorization: `Bearer ${directusToken}` } });
    const found = await findRes.json();
    let directusUser = null;
    const directusPayload = {
      email,
      first_name: (body == null ? void 0 : body.first_name) || (body == null ? void 0 : body.displayName) || null,
      last_name: (body == null ? void 0 : body.last_name) || null,
      firebase_uid: decoded.uid
    };
    if (findRes.ok && ((_a = found == null ? void 0 : found.data) == null ? void 0 : _a.length)) {
      const id = found.data[0].id;
      const updateUrl = `${directusUrl.replace(/\/$/, "")}/items/users/${id}`;
      const updateRes = await fetch(updateUrl, {
        method: "PATCH",
      ...
      directusUser = await updateRes.json();
    } else {
      const createUrl = `${directusUrl.replace(/\/$/, "")}/items/users`;
      const createRes = await fetch(createUrl, {
        method: "POST",
      ...
      directusUser = await createRes.json();
    }
    let magentoCustomer = null;
    if (magentoUrl && magentoToken) {
      try {
        const magentoResp = await fetch(`${magentoUrl.replace(/\/$/, "")}/rest/V1/customers`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${magentoToken}` },
          body: JSON.stringify({ customer: { email, firstname: directusPayload.first_name, lastname: directusPayload.last_name } })
        });
        magentoCustomer = await magentoResp.json();
        if (magentoCustomer && magentoCustomer.id && ((_b = directusUser == null ? void 0 : directusUser.data) == null ? void 0 : _b.id)) {
          const uid = directusUser.data.id;
          const patchUrl = `${directusUrl.replace(/\/$/, "")}/items/users/${uid}`;
          await fetch(patchUrl, {
            method: "PATCH",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${directusToken}` },
            body: JSON.stringify({ magento_customer_id: magentoCustomer.id })
          });
        }
      } catch (e) {
        magentoCustomer = { error: String(e) };
      }
    }
    return { directus: directusUser, magento: magentoCustomer };
  } catch (err) {
    event.res.statusCode = 500;
    return { error: (err == null ? void 0 : err.message) || "Sync failed" };
  }
});

const user_post$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: user_post
}, Symbol.toStringTag, { value: 'Module' }));
