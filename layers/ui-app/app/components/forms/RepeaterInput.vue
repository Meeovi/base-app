  ...
  <div :style="'width: ' + (props.width === 'full' ? '100%' : '50%')">
    ...
    <div v-if="!jsonMode">
      <div v-for="(it, idx) in items" :key="idx" style="border:1px solid #eee;padding:8px;margin-bottom:8px">
        <div v-if="props.options && Array.isArray(props.options.fields)">
          <div style="display:flex;flex-direction:column;gap:8px">
            <DirectusFormElement v-for="sub in props.options.fields" :key="sub.field" :field="sub" v-model="items[idx][sub.field]" />
          </div>
        </div>
        <div v-else style="display:flex;gap:8px;align-items:center;margin-bottom:6px">
    ...
    <div v-else>
      <textarea style="width:100%;min-height:200px;padding:8px" :value="JSON.stringify(items, null, 2)" @input="(e) => { try{ const parsed = JSON.parse(e.target.value); emit('update:modelValue', parsed); items.value = Array.isArray(parsed) ? parsed : []; } catch (err) { /* ignore invalid json until saved */ } }"></textarea>
    </div>
  </div>
</template>
